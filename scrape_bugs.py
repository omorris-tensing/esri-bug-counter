...
not based on its description text, and writes:
  ...
  - bug_counts.csv      : one row per patch with total + security-labeled counts
...
def write_report(counts: pd.DataFrame, detail: pd.DataFrame) -> None:
    ...
    lines.append("## Charts\n")
    lines.append("Run `python make_charts.py` to (re)generate the four McKinsey-style charts in "
                 "`charts/`. They are intentionally rendered as standalone PNGs so they can be "
                 "dropped into a slide deck. Each chart carries its own source line and "
    ...
    lines.append("| Security-labeled bug fixes by product | `charts/product_breakdown.png` | Portal for ArcGIS and ArcGIS Server account for nearly all security fixes |")
    lines.append("| Timeline of patches, sized by security-fix count | `charts/timeline.png` | Each dot is a patch; size is the number of security fixes it introduces |")
    lines.append("")
...
def main() -> int:
    ...
    session.headers.update(
        {
            "User-Agent": "esri-bug-counter/1.0 (research; contact: user@example.com)",
